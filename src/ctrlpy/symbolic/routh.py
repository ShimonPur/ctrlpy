...
class RouthArray:
    r"""Step-by-step pedagogical Routh-Hurwitz stability analyzer.

    Constructs the complete Routh array analytically using SymPy, handles edge cases
    (leading zero substitution with $\epsilon > 0$, rows of all zeros with auxiliary polynomial
    differentiation), evaluates RHP pole counts, and solves for parametric stability intervals.

    Parameters
    ----------
    poly_or_sys : TransferFunction | Sequence[float | sp.Basic] | NDArray | sp.Expr | sp.Poly
        Characteristic polynomial coefficients (highest degree first), polynomial expression,
        or a continuous-time TransferFunction.
    variable : str | sp.Symbol, default="s"
        Laplace complex frequency variable symbol.
    k_symbol : str | sp.Symbol | None, default=None
        Optional parametric gain symbol to solve for closed-loop stability intervals.

    Attributes
    ----------
    table : list[list[Any]]
        The complete computed 2D Routh array containing numeric or symbolic expressions.
    row_labels : list[str]
        Labels for each row of the Routh array (e.g. `['s^3', 's^2', 's^1', 's^0']`).
    num_rhp_poles : int
        Number of roots in the open Right-Half Plane (RHP). Equal to sign changes in column 1.
    is_stable : bool
        True if all roots lie strictly in the open Left-Half Plane (LHP).
    k_range : str | None
        Parametric stability range for symbolic gain $K$, if applicable.
    auxiliary_polynomials : list[sp.Expr]
        Any auxiliary polynomials formed when encountering rows of all zeros.
    steps : list[str]
        Pedagogical step-by-step construction notes.
    polynomial : Any
    ...
    def __init__(
        ...
        if isinstance(poly_or_sys, TransferFunction):
            if k_sym is not None:
                ...
                num_poly = sum(
                    c * s ** (len(poly_or_sys.num) - 1 - idx)
                    for idx, c in enumerate(poly_or_sys.num)
                )
                den_poly = sum(
                    c * s ** (len(poly_or_sys.den) - 1 - idx)
                    for idx, c in enumerate(poly_or_sys.den)
                )
                ...
                poly_display = sum(c * s ** (len(coeffs) - 1 - idx) for idx, c in enumerate(coeffs))
        ...
        # Auto-detect parametric gain symbol if present
        if k_sym is None:
            free_syms = set()
            for c in coeffs:
        ...
        self.polynomial = poly_display
        self.variable = s
        self.k_symbol = k_sym

        detailed_derivations.append(
            f"**Characteristic Equation:** $P({s}) = {sp.latex(poly_display)} = 0$ (Degree $n = {n}$)"
        )

        if n == 0:
            self.table = [[coeffs[0]]]
            self.num_rhp_poles = 0
            self.is_stable = True
            self.k_range = None
            self.steps = ["Degree 0 polynomial (constant)."]
            self.row_labels = ["s^0"]
        ...
        row0_str = ", ".join(f"${sp.latex(x)}$" for x in row0)
        row1_str = ", ".join(f"${sp.latex(x)}$" for x in row1)
        detailed_derivations.append(
            f"**Row ${row_labels[0]}$ Initialization:** [{row0_str}] (alternating even/odd coefficients)"
        )
        detailed_derivations.append(
            f"**Row ${row_labels[1]}$ Initialization:** [{row1_str}] (alternating coefficients)"
        )
        ...
        for i in range(2, n + 1):
            ...
            # Special Case 1: First element in previous row is 0, but row is not all zeros
            if _is_zero(prev_row[0]) and not all(_is_zero(x) for x in prev_row):
                ...
                note1 = (
                    ...
                    r"Replaced with symbolic $\epsilon > 0$."
                )
                steps.append(note1)
                detailed_derivations.append(f"**{note1}**")

            # Special Case 2: Previous row is all zeros
            if all(_is_zero(x) for x in prev_row):
                ...
                note2 = (
                    ...
                    f"Formed auxiliary polynomial $A({s}) = {sp.latex(aux_poly)}$ from row {row_labels[i - 2]} "
                    r"and replaced zero row with $\frac{dA}{ds}$ coefficients."
                )
                steps.append(note2)
                detailed_derivations.append(f"**{note2}**")

            # Compute row i entries: R[i, j] = (R[i-1, 0]*R[i-2, j+1] - R[i-2, 0]*R[i-1, j+1]) / R[i-1, 0]
            new_row: list[Any] = []
            a = prev_row[0]
            c = prev_prev_row[0]
            row_derivations: list[str] = []
            ...
            for j in range(num_cols):
                ...
                row_derivations.append(
                    rf"c_{{{j}}} = \frac{{({sp.latex(a)})({sp.latex(b)}) - ({sp.latex(c)})({sp.latex(d)})}}{{{sp.latex(a)}}} = {sp.latex(entry)}"
                )
            ...
            if all(_is_zero(x) for x in new_row) and i <= n:
                ...
                note2_new = (
                    ...
                    f"Formed auxiliary polynomial $A({s}) = {sp.latex(aux_poly)}$ from row {row_labels[i - 1]} "
                    r"and replaced with $\frac{dA}{ds}$ coefficients."
                )
                steps.append(note2_new)
                detailed_derivations.append(f"**{note2_new}**")
                new_row = deriv_row

            table.append(new_row)
            entries_str = ", ".join(f"${sp.latex(x)}$" for x in new_row)
            detailed_derivations.append(f"**Row ${pwr_label}$ Computation:** [{entries_str}]")
        ...
        if has_unresolved_param:
            ...
            detailed_derivations.append(
                rf"**Parametric Gain Analysis:** Solved column 1 positivity for ${k_sym} \implies$ **{k_range}**"
            )
        else:
            num_rhp = 0
            ...
            is_stable = (
                (num_rhp == 0)
                and (len(auxiliary_polys) == 0)
                and (len(non_zero_signs) == len(signs))
            )
            col1_tex = ", ".join(f"${sp.latex(x)}$" for x in col1)
            steps.append(f"Column 1 signs: {signs} -> {num_rhp} sign changes detected.")
            detailed_derivations.append(
                rf"**First Column Entries:** [{col1_tex}] with signs {signs} $\implies$ **{num_rhp} sign change(s)**."
            )
            conclusion_text = (
                "**Stability Conclusion:** Strictly Asymptotically Stable (All roots in open LHP)."
                if is_stable
                else (
                    f"**Stability Conclusion:** Unstable / Marginally Stable with **{num_rhp} Right-Half Plane (RHP) pole(s)**."
                )
            )
            detailed_derivations.append(conclusion_text)

        self.table = table
        self.num_rhp_poles = num_rhp
        self.is_stable = is_stable
        self.k_range = k_range
        self.steps = steps
        self.row_labels = row_labels
        self.auxiliary_polynomials = auxiliary_polys
        self._detailed_derivations = detailed_derivations

    def explain_steps(self) -> list[str]:
        """Return a step-by-step list of Markdown/LaTeX explanations detailing the array derivation.

        Returns
        -------
        list[str]
            List of explanatory step descriptions for each phase of the
            Routh-Hurwitz construction and stability evaluation.
        """
...
# Backwards compatibility aliases and functional interface
RouthResult = RouthArray
...
def routh_array(
    ...
    """Construct a RouthArray analyzer for a polynomial or dynamic system.

    Parameters
    ----------
    poly_or_sys : TransferFunction | Sequence[float | sp.Basic] | NDArray | sp.Expr | sp.Poly
        Characteristic polynomial coefficients, expression, or TransferFunction.
    variable : str | sp.Symbol, default="s"
        Laplace frequency variable symbol.
    k_symbol : str | sp.Symbol | None, default=None
    ...
    -------
    RouthArray
        The computed Routh-Hurwitz stability array.
    """
    return RouthArray(poly_or_sys=poly_or_sys, variable=variable, k_symbol=k_symbol)
...
def routh_table(
    ...
    -------
    RouthArray
        Computed Routh array object with table, stability metrics, and derivations.
    """
    return RouthArray(poly_or_sys=poly_or_sys, variable=variable, k_symbol=k_symbol)
